/**
 * Panoptica365 — Security Settings Page (Phase A1)
 *
 * Read-only list view over /api/security/tenants/:tid/settings.
 * Matches existing page-script patterns: IIFE, init() as the entry point,
 * wires button handlers, fetches data, renders into the partial's DOM.
 *
 * Phase A1 scope: tenant picker → table render → row click → Overview modal.
 * No Apply, no Match, no Remediate — those route to Phase B.
 */
(function () {
  'use strict';
  ...
  async function init(params = {}) {
    ...
    // Apr 27 — audit-only Match handler (CMP-02 DLP). Same endpoint as the
    // standard Match handler but a separate button so the audit-only panel
    // can show its own busy state and status.
    document.getElementById('sec-cfg-audit-match-btn').addEventListener('click', onAuditMatchClick);

    // Apr 28 — delegated-auth UI handlers (TEA-01, TEA-02). Sign-in opens
    // a popup to /auth/teams-delegated/login; popup posts back when done.
    ...
    window.addEventListener('message', (ev) => {
      ...
      // Defensive: validate auth status by re-fetching via session cookie
      // (don't trust the popup's payload). The renderer reads the live
      // status anyway.
      refreshDelegatedAuthBanner();
      if (data.ok) {
        // Mark the time of successful auth so the Apply circuit breaker can
        // detect "we just authenticated and Apply still 401s" and avoid
        // looping the popup. Window-attached so the flag survives across
        // module-reload edge cases (and is trivially cleared on real failures).
  ...
  // Standard Panoptica page entry — SPA router (app.js::loadPageScript)
  // reads window.PanopticaPage after script load and calls .init(params).
  window.PanopticaPage = {
    init,
    destroy: () => {
      // Clear any in-flight refresh-status polling so the interval doesn't
      // keep firing after the SPA has navigated away from this page.
      clearRefreshPolling();
    },
  };
})();
